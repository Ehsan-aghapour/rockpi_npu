// it is not bad to have a look at this page:
//https://github.com/tigert1998/npu_benchmark_model/blob/master/rknn_benchmark_model/jni/rknn_api_wrapper.cc
...
void print_output2(rknn_output output,int n){
	//n=std::min(n,400);
	float max=0;
	int max_indx=0;
	float* t=(float*)(output.buf);
...
void set_max_freq(){
	static bool governor_set=false;
	...
	cmd="echo " + std::to_string(LFreq) + " > /sys/devices/system/cpu/cpufreq/policy0/scaling_setspeed";
	system(cmd.c_str());
	//Set Big CPU Frequency
	cmd="echo " + std::to_string(BFreq) + " > /sys/devices/system/cpu/cpufreq/policy4/scaling_setspeed";
	system(cmd.c_str());
	//Set GPU Frequency
	cmd="echo " + std::to_string(GFreq) + " > /sys/devices/platform/ff9a0000.gpu/devfreq/ff9a0000.gpu/userspace/set_freq";
	system(cmd.c_str());
	cmd="cat /sys/devices/system/cpu/cpufreq/policy0/cpuinfo_cur_freq";
	system(cmd.c_str());
	cmd="cat /sys/devices/system/cpu/cpufreq/policy4/cpuinfo_cur_freq";
	system(cmd.c_str());
	cmd="cat /sys/devices/platform/ff9a0000.gpu/devfreq/ff9a0000.gpu/cur_freq";
	system(cmd.c_str());
	std::cerr<<"Maximum Frequency was set for GPU and CPU clusters\n";
...
int main(int argc, char *argv[]){
	...
	int ret = rknn_init(&ctx, model, model_len, init_flag);
	...
	printf("\n\n*************\n");
	print_output2(outputs[0],output0_attr.n_elems);
	...
	for(int i=0;i<n;i++){
		...
		std::cerr<<"run iteration: "<<i<<std::endl;
