...
//#define capture_freqs _IOR(Pandoon_MAGIC,'c',struct f*)
...
////static timespec last_swap;ehsan
...
void *new_frame_thread(void *tt) {
	...
	//uint64_t frames_time[10];
	////static int frame_number=0;
	//static uint64_t max_frame_time=0;
	static bool pass=0;
	...
	{	
		...
		if (fddd!=-1){
			//ALOGI("88 inside if pandoon opened(fddd)");
			//LOGI("pandoon interface opened succesfull!");
			////__android_log_print(ANDROID_LOG_INFO, "pandoon", "pandoon has opened!");
			//int PID=getpid();
			//const char* name;
			...
			if(!pass){
				//ALOGI("99 inside if not pass");

				/*chech size of ioctl data, it should match with kernel ioctl data size
				int ss=_IOC_SIZE(next_state);
				ALOGI("size of next_state:%d",ss);

                                ss=_IOC_SIZE(Apply_freqs);
                                ALOGI("size of Apply_freqs:%d",ss);

                                ss=_IOC_SIZE(capture_freqs);
                                ALOGI("size of caputre_freqs:%d",ss);

                                ss=_IOC_SIZE(capture_data);
                                ALOGI("size of capture_data:%d",ss);*/

				
				/* test next state ioctl:
				int jj=ioctl(fddd,next_state);
				ALOGI("next_state ioctl:%d",j);
				if(jj<0){
					ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				}*/
			
				/* test capture freqs ioctl:
				jj=ioctl(fddd,capture_freqs, &freqs);
				ALOGI("first capture_freqs:%d",jj);
				if(jj<0)
					ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));*/
				...
				dfreqs[1]=i1;
				dfreqs[2]=i2;				
				jj=ioctl(fddd, Apply_freqs,&dfreqs);
                                ALOGI("simple Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				...
				tp.padding=0;
				tp.a=dfreqs;
				jj=ioctl(fddd, Apply_freqs,tp.a);
                                ALOGI("union with tp.a Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				...
				t0_0.padding=0;
				t0_0.a=&(tp.a);
				jj=ioctl(fddd, Apply_freqs,t0_0.a);
                                ALOGI("union with t0_0.a Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				
                                t0_0.padding=0;
                                t0_0.a=&(tp.a);
                                jj=ioctl(fddd, Apply_freqs,t0_0);
                                ALOGI("union with t0_0 Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				...
				tp.padding=0;
				tp.a=dfreqs;
				jj=ioctl(fddd, Apply_freqs,tp);
                                ALOGI("union with tp Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				...
				t1_0.padding=0;
				t1_0.a=&(tp);
				jj=ioctl(fddd, Apply_freqs,t1_0);
                                ALOGI("union with t1_0 Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				...
				t1_0.padding=0;
				t1_0.a=&(tp);
				jj=ioctl(fddd, Apply_freqs,t1_0.a);
                                ALOGI("union with t1_0.a Apply_freqs:%d",jj);
                                if(jj<0)
                                        ALOGI("ioctl error error:%d,meaning: %s",errno,strerror(errno));
				*/
			...
			if(freqs.capturing or pass){
				...
				if(!myfileout.is_open()){
                                        //ALOGI("11 inside if my file data12 is not opened");
                                        std::string nnn="/data/out" + std::to_string(fff) + ".csv";
                                        myfileout.open(nnn);
                                }
                                if(!myfileout.is_open())
                                {
                                        ALOGI("Cannot Open out file at /data/ (Maybe Permisson Problem), Error: %s \n Trying /data/dataset/ Dir",strerror(errno));
                                        //__android_log_print(ANDROID_LOG_INFO, "pandoon", "data/Data.csv file: %s",strerror(errno));
                                        myfileout.open("/data/dataset/out"+ std::to_string(fff)+".csv");
                                }
                                if(!myfileout.is_open()){
                                        ALOGI("Cannot open myfile out at /data/dataset too, Error:%s.",strerror(errno));
                                        //return -1;
                                        mtxx.unlock();
                                        //close(fddd);
                                        return NULL;
				...
				*/
				...
				else
				{
					...
					//int jjj=ioctl(fdddkernel,capture_data,&data_state);
					...
					const char *mname="nm1";
					const char *mpath="/data/dataset/nm1.cambricon";
					HIAI_ModelBuffer* mb=HIAI_ModelBuffer_create_from_file(mname,mpath,HIAI_DEVPREF_HIGH);
					HIAI_ModelBuffer *modelBufferArray[] = {mb};
					if(mb == NULL){
						ALOGI("Model Buffer is NULL");
					}
					...
					//ofstream myfile;
					//myfile.open("/data/data/com.termux/files/home/input.csv");
					//float dataBuff[474];
					
					inputtensor = HIAI_TensorBuffer_create(input_N, input_C, input_H, input_W);
					inputtensor2 = HIAI_TensorBuffer_create(input_N, input2_C, input2_H, input2_W);
					inputtensorbuffer[0]= inputtensor;
					inputtensorbuffer[1]= inputtensor2;
					outputtensor = HIAI_TensorBuffer_create(output_N, output_C, output_H, output_W);
					outputtensorbuffer[0] = outputtensor;
					inputbuffer = (float *) HIAI_TensorBuffer_getRawBuffer(inputtensor);
					inputbuffer2 = (float *) HIAI_TensorBuffer_getRawBuffer(inputtensor2);
					//int length = HIAI_TensorBuffer_getBufferSize(inputtensor);
					//int length2 = HIAI_TensorBuffer_getBufferSize(inputtensor2);
					//ALOGI("SYNC JNI runModel modelname:%s", modelName);
					//memcpy(inputbuffer, dataBuff, length);
					//memcpy(inputbuffer2, dataBuff2, length2);
					...
					int ret2 = HIAI_ModelManager_runModel(
					    modelManager,
					    inputtensorbuffer,
					    input_Number,
					    outputtensorbuffer,
					...
					//clock_gettime(CLOCK_MONOTONIC, &t1);
					outputBuffer = (float *) HIAI_TensorBuffer_getRawBuffer(outputtensor);
					outputsize = output_N * output_C * output_H * output_W;
					
					//clock_gettime(CLOCK_MONOTONIC, &t2);
					//uint64_t out_getting_time=diff_time(t1,t2);
					//ALOGI("output getting time:%lu us\n",(long unsigned int)(out_getting_time/1000));	
					...
					//ALOGI("17 printed output in init");
					is_init=true;
					ALOGI("Initialized!");
				}
			}
	...
	{	
		...
		if((frame_number+1)%3 ==0){
			...
			ptr_data.padding=0;
			ptr_data.a=data_state;
			//ioctl(fdddkernel,capture_data,data_state);
		...
		if(frame_number==8)
			...
			//float *inputbuffer2 = (float *) HIAI_TensorBuffer_getRawBuffer(inputtensor2);
			//int length = HIAI_TensorBuffer_getBufferSize(inputtensor);
			//int length2 = HIAI_TensorBuffer_getBufferSize(inputtensor2);
			//ALOGI("SYNC JNI runModel modelname");
			//memcpy(inputbuffer, dataBuff, length);
			//memcpy(inputbuffer2, dataBuff2, length2);
			int8_t dfreqs[3];
	...
	else
	{
		...
		if (modelBuffer != NULL) {
        	...
        	ALOGE("JNI unload model ret:%d", ret);
